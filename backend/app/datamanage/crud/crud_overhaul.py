...
from backend.app.datamanage.model import Overhaul


class CRUDOverhaul(CRUDPlus[Overhaul]):
    ...
    async def get_by_model_and_bezier(
            self,
            db: AsyncSession,
            product_model: str,
            check_bezier: str
        ...
        :return: 所有数据
        '''
        stmt = select(self.model).order_by(self.model.repair_level)
        where_list = []
    ...
    async def get_by_model_and_no(self, db: AsyncSession, product_model: str, product_no: str)-> Optional[str]:
        stmt = select(self.model.check_value)
        where_list = []
        where_list.append(self.model.product_model == product_model)
        where_list.append(self.model.product_no == product_no)
        if where_list:
            stmt = stmt.where(*where_list).order_by(self.model.repair_level.desc())
            stmt = stmt.limit(1)
        result = await db.execute(stmt)
        return result.scalars().first()


overhaul_dao: CRUDOverhaul = CRUDOverhaul(Overhaul)
