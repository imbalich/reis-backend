...
class CRUDConfiguration(CRUDPlus[Configuration]):
    ...
    async def get_material_name_and_code(
            self, db: AsyncSession, product_model: str = None, process_name: str = None
    ) -> Sequence[Any]:
        stmt = select(
            distinct(self.model.extra_material_name).label("extra_material_name"),
            self.model.extra_material_code.label("extra_material_code")
        )
        if product_model:
            stmt = stmt.where(self.model.prod_model == product_model)
        if process_name:
            stmt = stmt.where(self.model.process_name == process_name)
        stmt = stmt.order_by(self.model.extra_material_name)
        result = await db.execute(stmt)
        # 返回 namedtuple 或 Row 对象
