...
class Failure(DataClassBase):
    ...
    maintenance_location: Mapped[str] = mapped_column(
        String(128), nullable=True, comment='检修地点', name='overhaul_location'
    )
    last_maintenance_date: Mapped[str] = mapped_column(
        String(30), nullable=True, comment='检修出厂日期', name='overhaul_factory_date'
    )
    manufacturing_date: Mapped[str] = mapped_column(
        String(30), nullable=True, comment='新造日期', name='production_data'
    )
    ...
    discovery_location: Mapped[str] = mapped_column(
        String(128), nullable=True, comment='二级配属', name='allotment_two'
    )
    ...
    fault_location: Mapped[str] = mapped_column(
        String(128), nullable=True, comment='终判故障部位名称', name='fault_part_name'
    )
    ...
    final_fault_responsibility: Mapped[str] = mapped_column(
    ...
    fault_part_batch_no: Mapped[str] = mapped_column(String(255), nullable=True, comment='故障件批次号')
    fault_part_serial_number: Mapped[str] = mapped_column(String(255), nullable=True, comment='故障件序列号')
    replacement_part_number: Mapped[str] = mapped_column(String(500), nullable=True, comment='更换件编号')
    repl_part_batch_no: Mapped[str] = mapped_column(String(255), nullable=True, comment='更换件批次号')
    repl_part_serial_number: Mapped[str] = mapped_column(String(255), nullable=True, comment='更换件序列号')
    ...
    is_online: Mapped[str] = mapped_column(String(500), nullable=True, comment='在线/返修')
