        ...
        try:
            async with async_db_session() as db:
                ...
                for part in parts:
                    ...
                    # 仅当两个数据源都存在时才保留该部件
                    if ebom_data and product_date.year_days and product_date.avg_worktime and replace_data:
                        valid_parts.append(part)
                    else:
                        # 可选：添加日志记录缺失数据的部件
                        logger.warning(
                            f"部件 {part} 缺少必要数据 - EBOM: {bool(ebom_data)}, 产品信息: {bool(product_date)}, 必换件: {bool(replace_data)}")
                        pass
