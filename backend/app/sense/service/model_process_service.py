...
class ModelProcessService:
    ...
    async def model_create(data):
        ...
        for name, config in models.items():
            ...
            cv_f1 = cross_val_score(gs.best_estimator_, x_train, y_train,
                                           cv=StratifiedKFold(n_splits=5, shuffle=True, random_state=42),
                                           scoring='f1')
            cv_roc = cross_val_score(gs.best_estimator_, x_train, y_train,
                                            cv=StratifiedKFold(n_splits=5, shuffle=True, random_state=42),
                                            scoring='roc_auc')
            # 计算平均值和标准差
    ...
    async def shap_analysis(model_result, test_data):
        ...
        numerical_cols = test_data["numerical_cols"]

        for model_info in model_result["ranked_models"]:
            best_model = model_info["best_model"]
            X_test_raw = X_test.loc[X_test.index, categorical_cols].copy()
