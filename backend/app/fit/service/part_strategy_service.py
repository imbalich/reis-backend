...
from backend.app.fit.schema.base_param import DespatchParam, FailureParam, ProductParam, EbomParam, ReplaceParam, \
    RepairParam
from backend.app.fit.service.part_tag_process_service import part_tag_process_service
from backend.app.fit.utils.convert_model import convert_to_pydantic_models, convert_to_pydantic_model, \
    convert_dict_to_pydantic_model, convert_to_total_quantity
...
class PartStrategyService:
    ...
    @staticmethod
    async def part_tag_process(model: str, part: str, input_date: Union[str, date] = None) -> list[list]:
        """
        零部件级别，单型号&单零部件标签处理
        :param model: 产品型号
        :param part: 零部件物料编码
        ...
        async with async_db_session() as db:
            try:
                # 获取基础数据
                despatch_data = convert_to_pydantic_models(await despatch_dao.get_despatchs_by_model(db, model),
                                                           DespatchParam)
                failure_data = convert_to_pydantic_models(await failure_dao.get_by_model_and_part(db, model, part),
                                                          FailureParam)
                ...
                replace_data = convert_to_pydantic_models(await replace_dao.get_by_model_and_part(db, model, part),
                                                          ReplaceParam)
