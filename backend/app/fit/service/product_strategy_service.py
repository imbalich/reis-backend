...
class ProductStrategyService:
    ...
    @staticmethod
    async def model_tag_process(model: str, input_date: Union[str, date] = None) -> list[list]:
        """
        整机级别，单型号标签处理
        :param model: 产品型号
        ...
        if not fault_check:
            raise errors.DataValidationError(msg=f"型号{model}的故障信息数量不足")
        # 3.检查累计运行时间Despatch
    ...
    @staticmethod
    async def models_tag_process(models: list[str], input_date: Union[str, date] = None) -> dict[str, Any]:
        """
        整机级别，多型号标签处理
        :param models: 产品型号列表
        :param input_date: 输入日期，格式为 "YYYY-MM-DD" 的字符串或 date 对象，默认为当前日期
