...
from backend.app.datamanage.model import Despatch, Failure, Product, Ebom
from backend.app.fit.schema.base_param import DespatchParam, EbomParam, FailureParam, ReplaceParam, RepairParam
...
class PartTagProcessService(TagProcessService):
    async def process_data(
        ...
        if "replace_data" not in kwargs:
            ...
            # 必换件
            return await self._process_essential(despatch_data, failure_data, product_data, bom_data,
                                                 kwargs["replace_data"], kwargs["repair_data"],
                                                 kwargs["repair_despatch_data"], input_date)
    ...
    async def _process_essential(
        ...
        container = await self.container_inspect(container, failure_data, bom_data)
        # 3.等级修发运填充
        container, error_info_list_repair = await self.container_insert_essential_repair(container,
    ...
    @staticmethod
    async def container_inspect(container: dict[str, Any], failure_data: list[FailureParam], bom_data: EbomParam) -> \
            dict[str, Any]:
        # 故障数据补充容器
    ...
    @staticmethod
    async def container_insert_non_essential(container: dict[str, Any], failure_data: list[FailureParam]) -> tuple[
        dict, list]:
        """
        非必换件故障数据插入
        :param container: 容器
        ...
        for failure in failure_data:
            # # 检查产品编号是否存在
            # if failure.product_number not in container['part_container']:
            #     error_info_message = f'型号 {failure.product_model} 部件 {failure.fault_location} 物料编码 {failure.fault_material_code} 故障报告ID {failure.report_id} 故障插入失败,产品编号 {failure.product_number} 不存在'
            ...
            for vmc, pt in container['part_container'][failure.product_number]['sub_container'].items():
                ...
                if (len(pt['fault_part_list']) > 0 and
                        pt['fault_part_list'][-1] == failure.fault_part_number
                        and not is_used):
            ...
            if not is_used:
                container['part_container'][failure.product_number]['sub_container'][
                    failure.fault_material_code + '-1']['fault_part_list'].append(failure.replacement_part_number)
                container['part_container'][failure.product_number]['sub_container'][
                    failure.fault_material_code + '-1']['fault_date_list'].append(
                    (failure.discovery_date, 'failure', 0))
                is_used = True
            if not is_used:
                # 插入失败
                error_info_message = f'型号 {failure.product_model} 部件 {failure.fault_location} 物料编码 {failure.fault_material_code} 故障报告ID {failure.report_id} 故障插入失败,请联系数字化管理部大数据室查询该故障信息完整性'
                error_info_list.append(error_info_message)
        return container, error_info_list

    @staticmethod
    async def tag_create_non_essential(container: dict[str, Any], product_data: Product, input_date: date) -> list[
        list[Any]]:
        result = []
            ...
            if despatch.identifier in container['part_container']:
                for vmc, pt in container['part_container'][despatch.identifier]['sub_container'].items():
                    ...
                    if despatch.life_cycle_time <= pt['fault_date_list'][-1][0]:
                        ...
                        error_info_message = f"产品 {despatch.model} 编号 {despatch.identifier} 等级修 {despatch.repair_level} 插入失败,时间 {despatch.life_cycle_time} 小于前值 {pt['fault_date_list'][-1][0]}"
                        error_info_list.append(error_info_message)
                        break
                ...
                for i in range(1, len(context['fault_date_list'])):
                    ...
                    if context['fault_date_list'][i][1] in ['repair', 'end']:
                        ...
                        if (diff.days - 90) <= product_data.repair_times:
                            ...
                            t = dateutils.run_time(product_data.repair_times - 90, product_data.year_days,
                    ...
                    if context['fault_date_list'][i][1] == 'failure':
                        ...
                        if (diff.days - 90) <= product_data.repair_times:
                            ...
                            t = dateutils.run_time(product_data.repair_times - 90, product_data.year_days,
                                                   product_data.avg_worktime)
                            cur = [bh, vt, context['fault_date_list'][i - 1][0],
                                   context['fault_date_list'][i][0],
                                   product_data.repair_times, t, 'suspense']
                            result.append(cur)
                            # 再插入一次故障
