...
@router.post(
    "/fit/swagger", summary="部件级别:创建单型号&单零部件数据拟合-->仅调试使用"
)
...
@router.post(
    "/fit-all", summary="部件级别:创建多型号+全部零部件数据拟合-->后台任务执行"
)
...
@router.post(
    "/fit-model-all", summary="部件级别:创建单型号+全零部件数据拟合-->后台任务执行"
)
async def part_create_fit_model_all_task(obj: CreateFitModelAllPartInParam):
    # 移除了 await 关键字delay() 方法会立即返回一个 AsyncResult 对象，而不会阻塞当前的异步函数。
    # 任务会在后台异步执行，而 API 会立即返回任务 ID 和其他相关信息
    task = part_fit_model_all_task.delay(obj.model, obj.input_date, obj.method)
    return response_base.success(
        data={
            "task_id": task.id,
...
async def part_get_fits(
    ...
    part: str = Query(..., description="零部件名称"),
    method: Annotated[
        FitMethodType | None, Query(description="拟合方法")
    ] = FitMethodType.MLE,
    input_date: Annotated[str | None, Query(description="计算截止日期")] = None,
    check: Annotated[
        FitCheckType | None, Query(description="拟合优度检验")
    ] = FitCheckType.BIC,
    source: Annotated[
        bool, Query(description="数据来源，False为系统默认，True为用户自定义")
    ] = False,
):
    """
    获取单个产品型号的拟合结果
