...
from typing import Sequence, Any, List, Type, TypeVar

import pandas as pd

from backend.app.fit.schema.base_param import EbomParam
from backend.app.fit.schema.fit_param import CreateProductDistributionParam, FitMethodType, CreatePartDistributionParam
...
def convert_to_product_distribution_params(
    ...
    for _, row in fit_results.iterrows():
        param = CreateProductDistributionParam(
            ...
            proportion_1=float(row['Proportion 1']) if pd.notna(row['Proportion 1']) and row[
                'Proportion 1'] != '' else None,
            ...
            log_likelihood=float(row['Log-likelihood']) if pd.notna(row['Log-likelihood']) and row[
                'Log-likelihood'] != '' else None,
            aicc=float(row['AICc']),
            bic=float(row['BIC']),
            ad=float(row['AD']),
