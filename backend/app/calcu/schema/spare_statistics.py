...
class PredictSpareRequest(SchemaBase):
    ...
    model_part_list: List[ModelPartItem] = Field(..., description="型号+零部件组合列表")
    input_date: date = Field(..., description="拟合输入日期")
...
class SpareStatisticsResultDetails(SchemaBase):
    ...
    start_date: date = Field(..., description="计算开始日期")
    end_date: date = Field(..., description="计算结束日期")
    predicted_spare_num: Optional[float] = Field(None, description="预计备件数量（精确小数）")
    predicted_spare_num_int: Optional[int] = Field(None, description="预计备件数量（取整整数）")
