...
async def fpmh(
    ...
    product_config_code: str | None = Query(None, description="派生码"),
    ...
    if distribution:
        distribution_obj = await distribute_service.get_distribution(
            model,
            part,
        ...
        if distribution_obj:
            result = await reliability_index_service.get_fpmh(
                model,
                part,
                t,
                distribution_obj,
